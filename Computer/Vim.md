...
Exコマンド内のバッククォートは二種類ある      
- コマンド置換      
`` ` ` ``で囲まれた文字列をUnixコマンドとして実行し、その結果で置き換える      
- 式の展開      
`` `= ` ``で囲まれた文字列をVimの式として展開し、その結果で置き換える      
※式とは http://vim-jp.org/vimdoc-ja/eval.html#expression-syntax   
   
Exコマンドには引数を、  
デフォルトで式として展開するコマンド（echoなど）と  
デフォルトで文字列として受け取るコマンド（tabeなど）があるので  
echoに文字列を渡すときはシングル・ダブルクォートで囲んだり、  
tabeの引数をバッククォートで囲んで式として展開してから実行するように手動で制御する必要がある。    
...
aaa.vimでccc()を実行すれば「aaa.vim」と出力されるし、bbb.vimでccc()を実行すれば「bbb.vim」と出力される。   
しかし、   
aaa.vimで`map <F2> :call s:ccc()`、   
bbb.vimで`map <F3> :call s:ccc()`   
とした場合、   
ユーザー定義関数として二つのccc()が登録され、見分けがつかなくなる。   
ユーザーが`<F2>`を押したとき、どっちのccc()が呼び出せば良いか判断できないので、   
mapするときはs:ではなく`<SID>`をつけると、動的にスクリプトID付きの関数名に変換してくれるので   
どっちのccc()かわかるようになる。   
   
### Plugとは   
仮想のキーストローク。   
プラグインを作成するときに利用する。   
aaa.vimというプラグインを作ったとき、その中の関数を利用できるようにaaa.vimの中で以下のようなmapを定義すべきのように思える。   
`map <F4> :call <SID>ccc()`   
しかし、プラグインを利用するユーザーが`<F4>`を既に利用していた場合、元の`<F4>`キーを押した動作ではなくなってしまう。   
   
それを防ぐために、aaa.vim内では   
`map <Plug>AaaCcc :call <SID>ccc()`   
としておく。   
`<Plug>AaaCcc`はキーボードからは入力できない仮想のキーストロークなので、ユーザーがすでに利用しているキーとかぶる事はない。   
   
もちろんこのままではユーザーがこのプラグインを使用できないので、   
ユーザー自身がコマンドラインモードや.vimrc等で   
`map <F5> <Plug>AaaCcc`   
とすれば、`<F5>`を押せば`<Plug>AaaCcc`を押したことになり、`:call <SID>ccc()`が実行できる。   
※これは入れ子キーマッピングを利用しているのでnoremapは使えない。   
...
http://vim-jp.org/vimdoc-ja/insert.html#compl-omni
...
- **補完・ジャンプをPHP用にする**
 ...
  外部コマンドとして`ctags -R`を実行する以下のキーマップを.vimrcで定義。  
  ...
  - PHPのインクルードの設定  
  デフォルトのincludeオプションが`include=\(require\|include\)\(_once\)\?`となっていたので特に設定なし。  
  - オムニ補完の設定  
...
 
# 実践
## read・write・filterを自由に使いこなしたい      
#### read   
- 現在のバッファに外部コマンドの結果を挿入する      
```
read !{CMD}
```   
   
- 現在のバッファを外部コマンドの結果で置き換える      
```
%d | read !{CMD}
```   
   
#### write   
- 現在のバッファの内容を標準入力として外部コマンドを実行し結果を標準出力で確認する      
```
write !{CMD}
```   
   
#### filter   
- 現在のバッファの内容を標準入力とした外部コマンドの結果で置き換える      
```
%!{CMD}   
※%は範囲指定なので可変である
```

- 現在のバッファの内容を標準入力とした外部コマンドの結果を別タブの一時ファイルに出力する  
```
let tmp_name = tempname() | w `=tmp_name` | tabe `=tmp_name` | %!{CMD}
※一時ファイルはvim標準のものであり、vimが終了すると自動で削除される
```
