...
コールバック関数... 他の関数やメソッドの引数として、関数やメソッドを渡す際の、渡される方を指す。  
可変関数... コールバック関数を実現するための方法の一つ。コールバック関数を受け取る側は関数名(foo)を$aaa受け取り、$aaa()と表現することで、「foo()」を実行できる。  
無名関数（クロージャ）... 関数名を省略した関数定義のこと。これをコールバック関数として渡すことができる。  
