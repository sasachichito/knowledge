※画像は「The DevOps ハンドブック 理論・原則・実践のすべて Kindle版」から引用
...
まずい品質保証の例  
```  
・面倒でエラーを起こしやすい手作業をほかのチームにしてもらわなければならないが、実際にはその仕事は簡単に自動化でき、作業を必要とするチームが必要に応じて実行できるはずのものである。  
・仕事から距離のある忙しい人物の承認が必要であり、その人物は仕事の内容や波及効果について十分な知識がないままに判断を下さざるを得ない。ただ承認印を押すだけで終わってしまうこともある。  
・疑問点を書き込んだ膨大なドキュメントを作るが、それは書いた直後からどんどん陳腐化していく。  
・承認を受けたり、作業してもらったりするために、特別な委員会やチームに大きなバッチで仕事を押し付け、返事を待っている。  
```  
そうではなく、バリューストリームに属するすべての人々が、日常業務の一部として自分の担当分野の問題を見つけて修正していく必要がある。  
そうすると、遠く離れた上級幹部の承認に頼るのではなく、実際に仕事を行なっている部門が品質や安全性について責任を持ち、意思決定するようになる。  
  
設計に対してなにがしかの保証が必要なら、提案された変更に対してピアレビューを行う。一般にはQAや情報セキュリティが行っている品質チェックをできる限り自動化する。  
特定の部門だけに品質の責任を押し付けるのではなく、すべての人が品質に責任を持つようになる。  
デベロッパーに自分が構築しているシステムの品質の責任を共有させると、作られるものが改善されるだけではなく、学習が加速される。  
  
- ### 下流のワークセンターのための最適な出力  
リーンは、設計の対象となる顧客として2つのタイプの人々を定義している。  
```  
社外の顧客... 私たちが提供するサービスのために料金を支払うだろう人  
社内の顧客... 私たちが作ったものを私たちのすぐあとで受け取り、加工する人  
```  
私たちのとって最も重要な顧客は、下流のすぐ次の位置にいる人たちだ。  
彼らの問題を自分の問題として考え、スムースで早いフローを妨げている設計上の問題をよりよく見極める必要がある。  
  
ITバリューストリームで下流のための最適な出力を提供するということは、運用のための最適な設計をするということだ。  
運用の非機能要件（アーキテクチャ・パフォーマンス・安定性・テスト可能性・構成/設定のしやすさ・セキュリティなど）にユーザー機能と同程度の優先順位を与える。  
  
こうすることで上流で高い品質を確保し、構築するすべてのサービスに一連の体系化された非機能要件を積極的に組み込めるようになる。  

# 第3の道：継続的な学習と実験の原則  
この原則は、継続的な学習と実験の文化を作り出す。  
これは個人がコンスタントに知識を生み出し、それをチームや組織の知識に転化していくための原則でもある。  

- ### 組織的な学習と安全尊重の文化を生み出す  
ITバリューストリームでは、安全な作業システムの確立を追求することが、発生的な文化を築くための基礎となる。  
事故や問題が起きた時には、ヒューマンエラーを犯した人物を追求するのではなく、事故の再発防止のためにシステムをどのように作り直すかを追求するのである。  
![組織タイプ](/picture/DevOps2.png "組織タイプ")

- ### 日常業務の改善を制度化する  
チームは作業プロセスを改善できなかったり、改善したがらないことが多い。  
Mike Rotherは「トヨタのカタ」の中で、改善されないからといってプロセスが同じままでいられないことを指摘している。  
カオスとエントロピーの増大により、プロセスは時間とともに劣化していくのである。  
「Lean IT」の著者Mike Orzenは「日常業務よりも大切なのは、日常業務の改善だ」と言っている。  
  
- ### ローカルな発見をグローバルな改善に転化する  
ある部門で新しい教訓を見つけたときは、社内の他の部門全体がその知識を活用し、利益を得られるようにするためのメカニズムも必要である。  
ITバリューストリームでは非難なしのポストモーテム（ふりかえり、事後検証）をサーチできるようにして、  
全社の最良の集合知を体現する共有ソースコードリポジトリーを作る。  
  
- ### 日常業務に回復力を鍛えるパターンを注入する  
日常業務に継続的に緊張を持ち込んだり、システムに回復力を組み込むために、アンチフラジャイルなプロセスをとる。  
```  
フラジャイル... 一回の大きな変化で大きな損を生み出すもの  
アンチフラジャイル... 障害が起きる前提で考え、変化による損が小さくなるもの  
  
アンチフラジャイルの概念を導入したNetflixはカオスエンジニアリングとして体現している。  
https://www.infoq.com/jp/news/2015/10/netflix-chaos-engineering  
```  
ITバリューストリームではデプロイリードタイムを削減し、テストカバレッジを拡大し、テストの実行時間を削減することを追求するためにアーキテクチャから作り直したり  
カオステストを導入する。  
  
- ### リーダーが学習する文化を奨励する  
偉大な成果を達成するためにはリーダーとワーカーの両方が必要である。  
リーダーはあらゆる問題解決で必要な「仕事との距離の近さ」がなく、一方の現場のワーカーは広い組織全体のコンテキストや自らの作業分野以外のことに変更を加える権威を持っていないため、どちらも自分たちだけでは仕事ができない。  
リーダーは視界が狭く権威も与えられていないワーカーの集団よりも、高いレベルでプロセスを設計し、動かす責任を持つ。  
  
リーダーの役割は、配下のチームが日常業務で偉大なものを見つけ出してくる条件を整えることにある。  
つまりリーダーは「事故0の持続」「1年以内に生産倍増」という非常に高い目標をはっきりと掲げるというものだ。  
このような戦略的な目標からは、短期的な目標の繰り返しが生まれる。  
短期的な目標は、バリューストリームレベルに下ろされ、「次の2週間以内にリードタイムを10%短縮する」といった状態目標に表現されて実施される。  
これらの状態目標は科学的実験の枠組みを形成する。  
  
リーダーは次のような質問を通じて実験を指揮する人物を指導する。  
```  
・最後のステップはどのようなもので、何が起きたか  
・あなたは何を学んだか  
・あなたの今の状態はどのようなものか  
・次の状態目標は何か  
・今はどのような障害を取り除こうとしているのか  
・あなたの次のステップは何か  
・あなたが予想する結果はどのようなものか  
・いつ結果をチェックできるか  
```  
このようにワーカーが日常業務の問題点を見つけ、解決するのをリーダーが助けるという問題解決アプローチこそ  
トヨタ生産方式、学習する組織、改善の型、高信頼性組織の核と言える。  
...
- **真正なリポジトリ**  
システムの正確な状態を格納する唯一の真正なリポジトリを用意する  
  
- **再構築できるインフラ**  
修理するよりも再構築する方が簡単なインフラストラクチャを作る  
  
- **完成の定義**  
本番に近い環境で動作することの確認も含めて「完成」というように、完成の定義を変える  
...
```  
- **小さなバッチ**  
小さなバッチによる開発についてとトランクへのコードのコミットが頻繁に行われない場合に何が起こるかについて  
...
# セキュリティ  
セキュリティ管理者は、変更管理プロセスを設けてチェックを行うことが多い。  
変更管理プロセスではアプリエンジニアとセキュリティ担当者とで職務が分離される。  
```  
「職務の分離」のデメリット：  
リードタイムを確実に伸ばす。  
仕事から遠い人物によるチェックのため効果が薄い。  
自分でやるより結果のフィードバックが当然少ない。  
自分の仕事に責任を持たなくなる。  
組織的な学習を生み出せなくなる。  
```  
アプリエンジニアは自分の仕事のセキュリティに関して責任を持てなくなり、組織的な学習も生み出せない。  
さらに外部の数少ないセキュリティ担当部門に頼りがちになり、結果精査する時間が足りずおろそかになりやすい。  
  
セキュリティ対策をITバリューストリーム全体に、つまりエンジニアの毎日の職務に組み込む必要がある。  
できればデプロイパイプラインに組み込んだり自動ツールで補いたい。  
そしてセキュリティを向上させるためだけでなく、  
規制や契約上の義務に対するコンプライアンスのための監査をしやすく、管理の効果を証明しやすいプロセスを作る。  
