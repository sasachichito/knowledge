...
# 拡張とモディフィケーション
```
標準クラスの拡張手段は次の2種類に分かれる。
モディフィケーション... 標準プログラムの変更（Change）
拡張... 標準プログラムの拡張（Add）

拡張は、コード内の事前定義された位置で実行することが可能。この位置を拡張オプションと呼ぶ。

拡張オプションには次の2種類がある。
・明示的な拡張オプション... 標準クラスの開発者が拡張用に使用可能なパラメータと共に明示的に定義した位置。
・暗黙的な拡張オプション... 標準クラスの一行目の前、最後の行の後のような位置。

明示的な拡張オプションには、ソースコードプラグインかBAdIを挿入可能である。

ソースコードプラグイン... 拡張対象の標準クラスに埋め込まれるソースコード
BAdI... 拡張対象の標準クラスから呼び出されるクラス。特定のインターフェースを実装している。
実装クラスが複数ある場合、条件分岐で動的に実行対象クラスを選択可能。

暗黙的な拡張オプションには、ソースコードプラグインを挿入可能である。

※暗黙的な拡張オプションでの拡張は、拡張用でない標準ロジックのパラメータを元に作成しているため、
標準ソースコードが変更されると動作しなくなる可能性がある
```
