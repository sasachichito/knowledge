...
という流れを守ることにより整合性が保たれ、クライアント1,2の可用性も維持されている。  
  
もしネットワーク障害によりAとBの間が分断された場合、この流れは実現できないのでABを停止するしかない。  
つまり分断耐性（P）が無い。(RDBはこのCAシステム)  
  
ではどうすればよいか。このときがCPとAPへの分かれ道となる。  
  
整合性を担保し続けるためには、AかBどちらかを停止する。仮にBを停止するとクライアント2は可用性を失う。（CPシステム）  
クライアントの可用性を担保するためには、AとBを稼働させ続ける。代わりにAB間のデータの整合性を失う。（APシステム）  
...
```  
これは単純なものでR（読み込み数）とW（書き込み数）の合計がN（ノード数）より大きい場合、必ず最新のデータを見つけることができるということである。  
例えば  
